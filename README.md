...
（請求間隔，預設 1500）、`SCRAPE_ORPHAN_ONLY`（只補爬「其他」孤兒教室，見下）。
同一門課跨多節／多天會以**流水號（`cr_cono`）**歸成一門、時段去重。

> **「其他」孤兒教室** — 體育課（網球場／球場／游泳池）、新生專題、外教課等只出現在
> 來源「全部」(`%`) 下、不屬於任何命名建物。爬完命名建物後會自動追加一個 **`其他`**
> pass，補爬這些教室。要單獨補爬可用 `SCRAPE_ORPHAN_ONLY=1 npm run scrape`（命名建物
> 只取教室清單以判斷哪些是孤兒，不重爬其課程）。爬蟲純 upsert、不刪課，部分補爬安全。
